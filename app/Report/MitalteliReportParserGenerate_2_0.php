...
/**
 * Class MitalteliReportParserGenerate_2_0 - parse a report.xml file and generate the report.
 */
class MitalteliReportParserGenerate_2_0 extends ReportParserGenerate
{
        ...
        switch ($listname) {
            case 'pending':
                ...
                foreach ($xrefs as $xref) {
                    ...
                    $row_tmp = Registry::gedcomRecordFactory()->make($xref, $this->getFromParentPrivatePropertyWithReflection('tree'));
                    $list[] = $row_tmp;
                ...
                if (preg_match("/filter(\d)/", $key)) {
                    ...
                    if (preg_match("/@(\w+)/", $condition, $match)) {
                        ...
                        if ($id === 'ID') {
                            ...
                            if (preg_match("/\d $id (.+)/", $this->getFromParentPrivatePropertyWithReflection('gedrec'), $match)) {
